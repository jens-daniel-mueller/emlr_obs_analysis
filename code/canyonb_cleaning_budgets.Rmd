---
title: "CANYON_B cleaning: Budgets"
...
for (i_Version_IDs in Version_IDs) {
  ...
  params_local <- bind_cols(
    ...
    CANYON_B_cruise_mean = params_local$CANYON_B_cruise_mean,
    CANYON_B_sample_SD = params_local$CANYON_B_sample_SD,
...
params_local_all <- params_local_all %>% 
  mutate(CANYON_B_cruise_mean = fct_inseq(as.factor(as.character(CANYON_B_cruise_mean))),
         CANYON_B_sample_SD = fct_inseq(as.factor(as.character(CANYON_B_sample_SD))))
...
dcant_budget_global_all %>%
  ggplot(aes(period, dcant, col=data_source)) +
  geom_point() +
  facet_grid(CANYON_B_cruise_mean ~ CANYON_B_sample_SD)
...
dcant_budget_global_bias_all %>%
  ggplot(aes(period, dcant_bias_rel)) +
  geom_hline(yintercept = 0) +
  geom_point() +
  facet_grid(CANYON_B_cruise_mean ~ CANYON_B_sample_SD,
             labeller = label_both)
...
params_local_all <- params_local_all %>% 
  mutate(rarefication_threshold = fct_inseq(as.factor(as.character(rarefication_threshold))))
