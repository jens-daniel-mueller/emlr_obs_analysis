---
title: "Rarefication: Column inventories"
...
```{r cases_absolute, fig.asp=1}

dcant_inv_all %>%
  filter(data_source %in% c("mod", "obs")) %>%
  group_by(period, data_source) %>%
  group_split() %>%
  head(3) %>%
  map(
    ~ p_map_cant_inv(df = .x,
...
dcant_inv_bias_all %>% 
  group_by(period, rarefication_threshold) %>% 
...
dcant_inv_all %>%
  ...
  map(
    ~ p_map_cant_inv(df = .x,
                     ...
                     subtitle_text = paste("period:",
                                           unique(.x$period))) +
      facet_grid(rarefication_threshold ~ data_source)
  )
