...
dcant_budget_global_all %>%
  ggplot(aes(period, dcant, col = gap_filling)) +
  geom_jitter(width = 0.2, height = 0) +
  scale_color_brewer(palette = "Dark2", name = "filters") +
...
dcant_budget_global_bias_all %>%
  ggplot(aes(period, dcant_bias, col = gap_filling)) +
...
p_global_bias <- 
  ...
  geom_point(aes(period, dcant_bias_rel, col = gap_filling),
             alpha = 0.7) +
    theme(axis.text.x = element_text(angle = 45, hjust=1),
        axis.title.x = element_blank())
...
dcant_budget_basin_AIP_bias_all %>%
  ...
  labs(y = expression(Delta ~ C[ant] ~ bias)) +
  theme(axis.title.x = element_blank()) +
  geom_jitter(aes(period, dcant_bias_rel, col = gap_filling),
              width = 0.05, height = 0) +
  facet_grid(basin_AIP ~ .)
...
dcant_slab_budget_all %>%
  filter(data_source == "obs",
         period != "1994 - 2014") %>% 
  ggplot(aes(gap_filling, dcant, fill = gamma_slab)) +
...
```{r slab_budgets_individual, fig.asp=1.5}
...
dcant_slab_budget_all %>%
  ...
  map(
    ~ ggplot(data = .x,
             aes(gap_filling, dcant, fill = gamma_slab)) +
      geom_hline(yintercept = 0) +
      geom_col() +
      scale_fill_scico_d(direction = -1) +
...
ggplot() +
  ...
  geom_point(
    ...
    aes(period, dcant, col = gap_filling),
    alpha = 0.7,
...
dcant_budget_global_all_in_sum <-
  ...
  filter(period != "1994 - 2014") %>%
  arrange(tref1) %>%
  group_by(data_source, gap_filling) %>%
...
ggplot() +
  geom_point(
    ...
    aes(estimate, dcant, col = gap_filling),
    alpha = 0.7,
...
dcant_budget_global_all_depth %>%
  ...
  map(
    ~  ggplot(data = .x,
              aes(dcant, gap_filling, fill=period)) +
...
dcant_budget_global_bias_all_depth %>%
  ggplot(aes(dcant_bias, gap_filling, fill = period)) +
  geom_vline(xintercept = 0) +
  geom_col(position = "dodge") +
  scale_fill_brewer(palette = "Dark2") +
  facet_grid(inv_depth ~ .)
...
dcant_budget_global_bias_all_depth %>%
  ggplot(aes(dcant_bias_rel, gap_filling, fill = period)) +
  geom_vline(xintercept = 0) +
  geom_col(position = "dodge") +
  scale_fill_brewer(palette = "Dark2") +
  facet_grid(inv_depth ~ .)
...
dcant_budget_basin_AIP_all_depth %>%
  ...
  map(
    ~  ggplot(data = .x,
              aes(dcant, gap_filling, fill = basin_AIP)) +
      geom_vline(xintercept = 0) +
      geom_col() +
      scale_fill_brewer(palette = "Dark2") +
...
dcant_budget_basin_AIP_bias_all_depth %>%
  ggplot(aes(dcant_bias, gap_filling, fill = basin_AIP)) +
  geom_vline(xintercept = 0) +
  geom_col() +
  scale_fill_brewer(palette = "Dark2") +
  facet_grid(inv_depth ~ period)
...
dcant_budget_basin_AIP_bias_all_depth %>%
  ggplot(aes(dcant_bias_rel, gap_filling, fill = basin_AIP)) +
  geom_vline(xintercept = 0) +
  geom_col(position = "dodge") +
  scale_fill_brewer(palette = "Dark2") +
  facet_grid(inv_depth ~ period)
