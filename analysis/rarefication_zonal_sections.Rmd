---
title: "Rarefication: Zonal sections"
...
```{r cases_SD_MLR_density}

dcant_zonal_all %>% 
  ggplot(aes(dcant_sd_mean, col=rarefication_threshold)) +
  geom_density() +
...
dcant_zonal_all %>% 
  group_by(period, basin_AIP, rarefication_threshold) %>% 
  summarise(dcant_sd_mean = mean(dcant_sd_mean, na.rm = TRUE)) %>% 
  ggplot(aes(rarefication_threshold, dcant_sd_mean)) +
  geom_point() +
  facet_grid(period ~ basin_AIP)
...
dcant_zonal_bias_all_corr <- dcant_zonal_bias_all %>%
  ...
  pivot_wider(names_from = period,
              ...
              names_prefix = "dcant_bias_")

dcant_zonal_bias_all_corr %>% 
  ggplot(aes(`dcant_bias_1994 - 2004`, `dcant_bias_2004 - 2014`)) +
...
dcant_zonal_bias_all %>% 
  ggplot(aes(dcant_bias, col=rarefication_threshold)) +
  geom_density() +
...
dcant_zonal_bias_all %>% 
  group_by(period, basin_AIP, rarefication_threshold) %>% 
  summarise(dcant_bias_sd = sd(dcant_bias)) %>% 
  ggplot(aes(rarefication_threshold, dcant_bias_sd)) +
  geom_point() +
  facet_grid(period ~ basin_AIP)
...
dcant_zonal_ensemble_bias <- full_join(
  ...
  dcant_zonal_all %>%
    filter(data_source == "mod_truth",
           MLR_basins == unique(dcant_zonal_all$MLR_basins[1])) %>% 
