...
for (i_Version_IDs in Version_IDs) {
  ...
  params_local <- bind_cols(
    ...
    cstar_corr_316N199 = params_local$cstar_corr_316N199,
...
params_local_all <-
  params_local_all %>%
  mutate(cstar_corr_316N199 = as.factor(cstar_corr_316N199))
...
dcant_budget_global_all %>%
  ggplot(aes(period, dcant, col = cstar_corr_316N199)) +
...
dcant_budget_global_all %>%
  ...
  ggplot(aes(as.numeric(as.character(cstar_corr_316N199)), dcant)) +
...
lm(dcant ~ corr,
   data = dcant_budget_global_all %>%
     filter(data_source == "obs") %>%
     mutate(corr = as.numeric(as.character(cstar_corr_316N199))))
...
dcant_budget_global_bias_all %>%
  ggplot(aes(period, dcant_bias, col = cstar_corr_316N199)) +
...
dcant_slab_budget_all %>%
  filter(data_source == "obs",
         period != "1994 - 2014") %>% 
  ggplot(aes(cstar_corr_316N199, dcant, fill = gamma_slab)) +
...
dcant_slab_budget_bias_all %>%
  ...
  scale_fill_scico_d(direction = -1) +
  facet_grid(basin_AIP ~ cstar_corr_316N199)
