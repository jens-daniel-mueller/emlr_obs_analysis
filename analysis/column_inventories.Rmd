...
cant_inv_all_bias <- cant_inv_all %>%
  select(lat, lon, data_source, Version_ID, era_duplication, MLR_basins, cant_inv) %>%
...
cant_inv_budget <- cant_inv_all %>% 
  ...
  group_by(basin_AIP, data_source, Version_ID, MLR_basins, era_duplication, duration) %>% 
...
```{r filter_standard_inventory_depth_subsetting}
...
```{r column_inventory_maps_output_subsetting, fig.asp=1}
