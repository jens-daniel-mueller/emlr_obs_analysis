...
dcant_budget_global_all %>%
  ggplot(aes(period, dcant, col=CANYON_B_sample_SD)) +
  geom_point() +
...
dcant_budget_global_bias_all %>%
  ggplot(aes(period, dcant_bias, col=CANYON_B_sample_SD)) +
  geom_hline(yintercept = 0) +
  geom_point()

dcant_budget_global_bias_all %>%
  ggplot(aes(period, dcant_bias_rel, col=CANYON_B_sample_SD)) +
  geom_hline(yintercept = 0) +
  geom_point()
...
ensemble_CANYON_B_sample_SD_global <- dcant_budget_global_bias_all %>% 
  filter(dcant_bias_rel < global_bias_rel_max) %>% 
  count(CANYON_B_sample_SD) %>% 
  filter(n == 2) %>% 
  pull(CANYON_B_sample_SD)
...
dcant_budget_global_ensemble <- dcant_budget_global_all %>% 
  filter(data_source %in% c("mod", "obs"),
         CANYON_B_sample_SD %in% ensemble_CANYON_B_sample_SD_global) %>% 
...
dcant_budget_global_all_depth %>%
  filter(CANYON_B_sample_SD %in% ensemble_CANYON_B_sample_SD_global) %>% 
  group_by(data_source) %>%
  group_split() %>%
  # head(1) %>%
  map(
    ~  ggplot(data = .x,
              aes(dcant, CANYON_B_sample_SD, fill=period)) +
...
dcant_budget_global_bias_all_depth %>%
  filter(CANYON_B_sample_SD %in% ensemble_CANYON_B_sample_SD_global) %>%
  ggplot(aes(dcant_bias, CANYON_B_sample_SD, fill = period)) +
  geom_vline(xintercept = 0) +
  geom_col(position = "dodge") +
  scale_fill_brewer(palette = "Dark2") +
  facet_grid(inv_depth ~ .)
...
dcant_budget_global_bias_all_depth %>%
  filter(CANYON_B_sample_SD %in% ensemble_CANYON_B_sample_SD_global) %>%
  ggplot(aes(dcant_bias_rel, CANYON_B_sample_SD, fill = period)) +
  geom_vline(xintercept = 0) +
  geom_col(position = "dodge") +
  scale_fill_brewer(palette = "Dark2") +
  facet_grid(inv_depth ~ .)
...
dcant_budget_basin_AIP_bias_all %>% 
  ...
  distinct(period, CANYON_B_sample_SD, basin_AIP)

ensemble_CANYON_B_sample_SD_regional <- dcant_budget_basin_AIP_bias_all %>% 
  filter(dcant_bias_rel < regional_bias_rel_max) %>% 
  count(CANYON_B_sample_SD) %>% 
  filter(n == 6) %>% 
  pull(CANYON_B_sample_SD)
...
dcant_budget_basin_AIP_ensemble <- dcant_budget_basin_AIP_all %>% 
  filter(data_source %in% c("mod", "obs"),
         CANYON_B_sample_SD %in% ensemble_CANYON_B_sample_SD_global,
         CANYON_B_sample_SD %in% ensemble_CANYON_B_sample_SD_regional) %>% 
...
unique(
dcant_budget_basin_AIP_all_depth$CANYON_B_sample_SD)
...
dcant_budget_basin_AIP_all_depth %>%
  ...
  map(
    ~  ggplot(data = .x,
              aes(dcant, CANYON_B_sample_SD, fill = basin_AIP)) +
      geom_vline(xintercept = 0) +
      geom_col() +
      scale_fill_brewer(palette = "Dark2") +
...
dcant_budget_basin_AIP_bias_all_depth %>%
  ...
  ggplot(aes(dcant_bias, CANYON_B_sample_SD, fill = basin_AIP)) +
  geom_vline(xintercept = 0) +
  geom_col() +
  scale_fill_brewer(palette = "Dark2") +
  facet_grid(inv_depth ~ period)
...
dcant_budget_basin_AIP_bias_all_depth %>%
  ...
  ggplot(aes(dcant_bias_rel, CANYON_B_sample_SD, fill = basin_AIP)) +
  geom_vline(xintercept = 0) +
  geom_col() +
  scale_fill_brewer(palette = "Dark2") +
  facet_grid(inv_depth ~ period)
