  ...
  map(
    ~ p_map_cant_inv(df = .x,
                     ...
                     subtitle_text = paste("period:",
                                           unique(.x$period))) +
      facet_grid(CANYON_B_sample_SD ~ data_source)
  )
...
dcant_inv_bias_all %>%
  ...
  facet_grid(CANYON_B_sample_SD ~ period)
...
dcant_inv_ensemble_member <- dcant_inv_bias_all %>% 
  group_by(period, CANYON_B_sample_SD) %>% 
  summarise(dcant_sd = sd(dcant_bias)) %>% 
  ungroup()

dcant_inv_ensemble_member %>% 
  ggplot(aes(CANYON_B_sample_SD, dcant_sd, col = period)) +
  geom_point()
