...
dcant_zonal_all %>%
  ...
  map(
    ~ p_section_zonal_continous_depth(
      ...
      facet_grid(cstar_deep_sd ~ data_source)
  )
...
dcant_zonal_bias_all %>%
  ...
  map(
    ~ p_section_zonal_continous_depth(
      ...
      facet_grid(cstar_deep_sd ~ period)
  )
...
dcant_zonal_bias_all %>%
  ggplot(aes(dcant_bias, col = cstar_deep_sd)) +
...
dcant_profile_all %>%
  ...
  map(
    ~ ggplot(data = .x,
      ...
      facet_grid(cstar_deep_sd ~ basin_AIP)
  )
...
dcant_profile_all %>%
  ...
  map(
    ~ ggplot(data = .x,
             aes(
               dcant, depth,
               col = cstar_deep_sd, fill = cstar_deep_sd
             )) +
      geom_hline(yintercept = params_global$inventory_depth_standard) +
      geom_vline(xintercept = 0) +
      geom_path() +
  ...
  map(
    ~ ggplot(
      ...
      aes(
        ...
        depth,
        col = period,
        group = interaction(cstar_deep_sd, period)
      )
    ) +
      geom_hline(yintercept = params_global$inventory_depth_standard) +
      geom_vline(xintercept = 0) +
