...
delta_dcant_inv_all_ensemble_sd <-
  ...
  summarise(delta_dcant_sd = sd(delta_dcant) * 2,
...
wrap_plots(p_dcant_global_section_dec1,
           p_dcant_global_section_dec2,
           p_dcant_global_section_offset,
           ncol = 1)
...
```{r exclude_Watson_from_GCB, eval=FALSE}
...
p_tcant_ts_zoom <-
  ...
  #           aes(pCO2, fgco2_glob_cum_total_mean, col = type),
  #           size = 1.5) +
  geom_point(
...
emission_components <- flux_components[c(1,2)]
...
estimates %>%
  ...
  pivot_wider(names_from = estimate,
              values_from = GtC_yr) %>%
  mutate(`land sink residual` = `fossil emissions excluding carbonation` + `land-use change emissions` + `atmospheric growth` + `ocean sink`) %>%
  ggplot() +
  geom_line(aes(year, -`land sink residual`, col = "residual")) +
  geom_line(aes(year, `land sink`, col = "direct"))
...
Ocean_Sink_annual_mean %>% 
  ggplot(aes(year, GtC_yr, col = type)) +
  geom_path()
...
estimates <-
  full_join(estimates, Ocean_Sink_annual_mean)
...
estimates <- estimates %>% 
  ...
  mutate(ocean_sink_interior_net = case_when(
    ...
    between(year, 2004, 2013) ~ -dcant_interior[1]-cnat[2],
    TRUE ~ NaN
  ))
...
estimates %>%
  ...
  pivot_longer(starts_with("ocean_sink"),
