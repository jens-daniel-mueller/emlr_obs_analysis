...
```{r inventory_maps_absolute_dcant_vs_SD}
...
dcant_budget_ensemble_stat_sensitvity_cases <-
  ...
  pivot_wider(names_from = Version_ID_group,
              values_from = dcant) %>%
  pivot_longer(
    -c(basin:data_source, unique(params_local_all$Version_ID_group)),
...
dcant_budget_ensemble_stat_factorial <-
  dcant_budget_ensemble_stat_factorial %>%
  filter(Version_ID_group %in% unique(params_local_all_ensemble$Version_ID_group)[c(1, 3:7)])
...
```{r beta_budget_basin_hemisphere_beta_uncertainty}
