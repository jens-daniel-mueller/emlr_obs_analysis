...
params_local <- params_local_back
...
ggsave(path = here::here("output/publication"),
       filename = "Fig_dcant_profiles_regional.png",
       height = 6,
       width = 8)
...
dcant_budget_global_all_ensemble %>%
  ...
  pivot_wider(names_from = data_source,
              values_from = dcant) %>% 
  mutate(dcant_bias_rel = 100*(mod - mod_truth)/mod_truth)
...
delta_dcant_budget_global_all_ensemble_bias %>% 
  group_by(data_source) %>% 
  summarise(delta_dcant = mean(delta_dcant)) %>% 
  ungroup()
...
ggsave(path = here::here("output/publication"),
       filename = "Fig_dcant_budget_global_ensemble_member.png",
       height = 4,
       width = 3)
...
ggsave(path = here::here("output/publication"),
       filename = "Fig_dcant_budget_regional_ensemble_member.png",
       height = 4,
       width = 7)
...
beta_budget_basin_MLR_all_ensemble %>% 
  ...
  scale_y_continuous(name = dcant_pgc_scaled_label, breaks = seq(0,5,0.05)) +
  facet_grid(basin ~ ., scales = "free_y")
...
beta_budget_ensemble_stat <-
  bind_rows(
    beta_budget_basin_MLR_all_ensemble,
    beta_budget_global_all_ensemble %>% mutate(basin = "global")
  ) %>%
  filter(data_source == "obs",
         period != "1994 - 2014") %>%
  group_by(period, basin) %>%
...
beta_budget_ensemble_stat <- beta_budget_ensemble_stat %>% 
  pivot_longer(c(mean,median,std_case),
               names_to = "estimate",
               values_to = "beta") %>%
  group_by(estimate, basin) %>%
