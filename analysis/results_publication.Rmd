...
```{r concentration_profiles_MLR_basins_ensemble_uncertainty_ribbon_mod, fig.asp=0.9, eval=FALSE}
...
dcant_budget_basin_MLR_all_ensemble_bias %>%
  filter(period %in% two_decades) %>%
  select(basin, period, MLR_basins, Version_ID_group, mod, mod_truth) %>%
...
delta_dcant_budget_basin_MLR_all_ensemble_bias %>% 
  ggplot(aes(period, delta_dcant_bias_rel, col = Version_ID_group)) +
  geom_hline(yintercept = 0) +
  geom_jitter(alpha = 0.5) +
  scale_color_brewer(palette = "Dark2") +
  facet_grid(basin ~ MLR_basins)
