...
```{r read_RECCAP2_flux_products_observations, eval=FALSE}
...
```{r read_SeaFlux_flux_products_observations, eval=FALSE}
...
Ocean_Sink_cum_1994 <- Ocean_Sink_cum_1994 %>% 
  mutate(fgco2_glob_cum = fgco2_glob_cum + 118) %>%
  select(pCO2, product, fgco2_glob_cum, type)
...
tcant_ts_zoom %>%
  ...
  geom_line(data = Ocean_Sink_cum_1994,
            aes(pCO2, fgco2_glob_cum, group = product, col = type)) +
