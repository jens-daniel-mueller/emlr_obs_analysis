...
```{r invetory_maps_absolute_test_color_scales, fig.asp=0.4}
...
```{r concentration_profiles, fig.asp=0.8}
...
```{r concentration_profiles_MLR_basins, fig.asp=1.2}
...
```{r concentration_profiles_basin_AIP_offset, fig.asp=0.8}
...
```{r concentration_profiles_MLR_basins_offset, fig.asp=1.2}
...
```{r depth_layer_budget_basin_AIP}
...
```{r depth_layer_budget_basin_MLR, fig.asp=1}
...
```{r depth_layer_budget_basin_MLR_cumulative, fig.asp=1}
...
dcant_budget_basin_MLR_layer_all %>%
  ...
  ungroup() %>% 
  ggplot(aes(inv_depth-250, dcant_cum, col=period)) +
  geom_hline(yintercept = 0) +
  geom_step(direction = "vh") +
  coord_flip() +
  scale_x_reverse(breaks = seq(0, 3000, 500)) +
