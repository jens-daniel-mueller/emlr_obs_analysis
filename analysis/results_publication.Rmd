...
beta <- 1.32
co2_atm_pi_value <- 277
tCant_col <- "red"

tcant_ts %>%
  ggplot(aes(year, (tcant / beta + co2_atm_pi_value))) +
  # geom_hline(yintercept = co2_atm_pi_value, col=tCant_col) +
  geom_line(data = co2_atm_reccap2 %>% filter(year > 1740),
            aes(year, pCO2)) +
  geom_point(fill = tCant_col, shape = 21, size = 5) +
  geom_text(aes(label = year), nudge_x = -10, nudge_y = 3) +
  scale_y_continuous(name = expression(Atm. ~ pCO[2] ~ (µatm)),
                     sec.axis = sec_axis(~ (. - 278) * beta,
                                         name = expression(Total ~ oceanic ~ C[ant] ~ (PgC)))) +
  scale_x_continuous(name = "Year", breaks = seq(1750, 2000, 50)) +
  theme(
...
tcant_ts %>% 
  ...
  geom_ribbon(aes(ymin = tcant - tcant_sd, ymax = tcant + tcant_sd, fill = "1750 - 2014"),
              alpha = 0.5) +
  ...
  geom_text(aes(label = year), nudge_x = -6, nudge_y = 5) +
...
RECCAP2_fgco2_glob_all_annual_cum_1994 <- RECCAP2_fgco2_glob_all_annual_cum_1994 %>% 
  mutate(fgco2_glob_cum = fgco2_glob_cum + 102,
         fgco2_glob_corr_cum = fgco2_glob_corr_cum + 102) %>%
...
tcant_ts %>%
  ...
  geom_line(data = RECCAP2_fgco2_glob_all_annual_cum_1994,
            aes(pCO2, fgco2_glob_corr_cum, group = product, col = "pCO2 observations")) +
  geom_linerange(aes(ymin = tcant - dcant_sd, ymax = tcant + dcant_sd)) +
  geom_line(aes(col = "eMLR(C*) estimate")) +
  scale_color_manual(values = c("black", "red", "blue"), name = "Data source") +
    scale_fill_manual(values = c("grey60", "grey30"), name = "Cumulative\nuncertainty") +
  labs(x = expression(Atmospheric ~ pCO[2] ~ (µatm)),
       y = expression(Total ~ oceanic ~ C[ant] ~ (PgC))) +
  theme(
