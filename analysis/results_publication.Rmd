...
```{r r distuinguish_standard_case_and_ensemble_MLR_stats, include=FALSE}
...
dcant_budget_basin_MLR_bias_all_decomposition_no_adj <- dcant_budget_basin_MLR_bias_all_decomposition %>% 
  filter(Version_ID %in% Version_IDs_d)
...
lm_best_predictor_counts_all_ensemble %>% 
  ...
  facet_grid(Version_ID_group~predictor)
...
lat_residual_all_ensemble_mean <- lat_residual_all_ensemble %>%
  filter(data_source == "obs",
         period %in% two_decades) %>% 
...
lat_residual_offset_all_ensemble_mean <- lat_residual_offset_all_ensemble %>%
  filter(data_source == "obs",
         period %in% two_decades) %>% 
...
ggplot() +
  geom_vline(xintercept = 0) +
  geom_ribbon(
    data = lat_residual_all_ensemble_mean,
    aes(
  ...
  geom_path(data = lat_residual_all_ensemble_mean,
            aes(residual_mean,
                lat_grid,
                col = era)) +
...
ggplot() +
  geom_vline(xintercept = 0) +
  geom_ribbon(
    data = lat_residual_offset_all_ensemble_mean,
    aes(
      xmin = residual_mean - residual_iqr,
      xmax = residual_mean + residual_iqr,
      y = lat_grid
  ...
  geom_path(data = lat_residual_offset_all_ensemble_mean,
            aes(residual_mean,
                lat_grid)) +
  coord_cartesian(xlim = c(-3,7)) +
  facet_grid(. ~ period)
...
lat_residual_offset_all_ensemble %>% 
...
# ggsave(path = here::here("output/publication"),
#        filename = "FigS_delta_dcant_column_inventory_ensemble_RSS.png",
#        height = 3,
#        width = 5)
...
ggplot() +
  geom_point(data = dcant_inv_all_uncertainty %>%
                filter(dcant_sign < 0,
                       data_source == "obs"),
              aes(lon, lat),
             shape = 4) +
...
```{r invetory_maps_delta_dcant_mod_bias, fig.asp=1.3}
...
```{r inventory_maps_absolute_delta_RSS}
...
ggplot() +
  geom_point(data = delta_dcant_inv_all_uncertainty %>%
...
```{r invetory_maps_absolute_mod, fig.asp=1.2}
...
```{r global_section_absolute_delta_RSS}
...
rm(delta_dcant_global_section_all_uncertainty)
...
```{r dcant_budget_stats_ensemble_SD}
...
dcant_budget_ensemble_stat_basin_SD <-
  ...
  filter(data_source == "obs",
         period != "1994 - 2014") %>%
  group_by(period, basin, Version_ID_group) %>%
...
dcant_budget_ensemble_stat_basin_SD <-
  dcant_budget_ensemble_stat_basin_SD %>%
  filter(Version_ID_group == "Bulk adjustment") %>%
...
dcant_budget_ensemble_stat_sensitvity_cases <-
  ...
  filter(data_source == "obs",
         period != "1994 - 2014") %>%
  select(basin, dcant, period, MLR_basins, Version_ID_group) %>%
  pivot_wider(names_from = Version_ID_group,
              values_from = dcant) %>%
  pivot_longer(
...
dcant_budget_ensemble_stat_sensitvity_cases %>% 
  ...
  labs(y = "Offset (PgC)",
       title = "Inventory offset compared to standard configuration",
...
dcant_budget_ensemble_stat_sensitvity_cases <-
  dcant_budget_ensemble_stat_sensitvity_cases %>%
  filter(MLR_basins == "3") %>%
  select(period, basin, Version_ID_group, sd = delta_dcant)

dcant_budget_ensemble_stat_factorial <-
  bind_rows(
    dcant_budget_ensemble_stat_basin_SD %>% mutate(type = "regional clustering",
                                                   Version_ID_group = "Bulk adjustment"),
...
dcant_budget_ensemble_stat_factorial %>%
  ggplot(aes(period, sd ^ 2, pattern = type, fill = Version_ID_group)) +
  geom_col_pattern(
  ...
  labs(
    y = expression("Squared uncertainty contribution (PgC" ^ 2 * ")"),
    title = "Uncertainty contributions from configuration and regional clustering",
    subtitle = "Note: Regional clustering contribution is based on bulk adjustment cases\nconfiguration changes are evaluated for the regional clustering scheme 3"
  ) +
  theme(axis.title.x = element_blank()) +
...
dcant_budget_basin_MLR_all_ensemble %>%
  ...
  geom_crossbar(
    data = dcant_budget_ensemble_stat_factorial,
    aes(
      x = period,
      y = std_case,
      ymin = std_case - 2*sd,
