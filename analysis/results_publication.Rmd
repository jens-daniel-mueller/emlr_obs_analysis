...
#   filter(period %in% two_decades,
#          data_source != "mod_truth") %>%
#   select(lat_grid, period, data_source, basin_AIP, MLR_basins, Version_ID_group, dcant)
...
delta_dcant_budget_lat_grid_uncertainty %>%
  ...
  scale_y_continuous(limits = c(-78,65),
                     ...
                     labels = c("60°S","30°S","Eq.","30°N","60°N"),
                     expand = c(0,0)) +
  scale_x_continuous(limits = c(-0.14,0.14), breaks = seq(-0.5, 0.6, 0.1),
                     expand = c(0,0)) +
  labs(x = expression((Pg~C~dec^{-1}~"1°lat"^{-1}))) +
  theme_classic() +
  theme(legend.title = element_blank(),
        legend.position = "bottom",
...
p_zonal_int_dec1 / p_zonal_int_dec2 / p_zonal_int_delta
...
p_dcant_inv_dec1 <-
  p_map_mdim_robinson(df = dcant_inv_dec1,
                      ...
                      var = "dcant",
                      legend_title = legend_title) +
  theme(legend.title = element_markdown(halign = 1,
                                        lineheight = 1.5)) +
  geom_label(aes(
    label = "1994 - 2004",
    x = lon_lim[1] * 0.55,
...
```{r dcant_budget_basin_hemisphere_additive_uncertainty_waterfall_new, fig.asp=1}
...
```{r dcant_budget_basin_hemisphere_additive_uncertainty_waterfall_new2, fig.asp=1}
