...
ggsave(path = here::here("output/publication"),
       filename = "FigS_dcant_ensemble_standard_deviation_column_inventory.png",
       height = 6,
       width = 6)
...
p_dcant_profiles <-
  ...
  coord_cartesian(expand = 0) +
  scale_color_manual(values = c("#d95f02", "#7570b3"), name = "Mean \u00B1 2 SD") +
  scale_fill_manual(values = c("#d95f02", "#7570b3"), name = "Mean \u00B1 2 SD") +
  labs(y = "Depth (m)",
       x = dcant_umol_label) +
  facet_wrap(~ basin, ncol = 3, dir = "v") +
...
dcant_budget_ensemble_significance %>% 
  drop_na() %>% 
  select(basin, estimate, delta_dcant:ttest_p) %>% 
  kable() %>%
  kable_styling() %>%
...
dcant_budget_basin_MLR_all_ensemble %>%
  ...
  scale_alpha_manual(name = "X", values = 0.5) +
...
beta_budget_ensemble_significance <- beta_budget_ensemble_stat %>% 
  ...
  group_by(estimate, basin) %>%
  mutate(
    delta_beta = beta - lag(beta),
    delta_beta_uncert = sqrt(2 * sd ^ 2 + lag(2 * sd ^ 2)),
    error_prop_ratio = abs(delta_beta_uncert / delta_beta),
    ttest_p = tsum.test(
      mean.x = beta,
      s.x = sd,
      n.x = n,
...
beta_budget_ensemble_significance %>% 
  drop_na() %>% 
  select(basin, estimate, delta_beta:ttest_p) %>% 
  kable() %>%
  kable_styling() %>%
  scroll_box(height = "300px")
...
budget_ensemble_significance <-
  bind_rows(
    beta_budget_ensemble_significance %>%
      ...
      relocate(parameter) %>%
      rename(
        offset_value = delta_beta,
        offset_uncertainty = delta_beta_uncert,
        uncert_value_ratio = error_prop_ratio
      ),
    dcant_budget_ensemble_significance %>%
      drop_na() %>%
      select(basin, estimate, delta_dcant:ttest_p) %>%
      mutate(parameter = "dcant") %>%
      relocate(parameter) %>%
      rename(
        offset_value = delta_dcant,
        offset_uncertainty = delta_dcant_uncert,
        uncert_value_ratio = error_prop_ratio
      )
  )

budget_ensemble_significance %>% 
  ggplot(aes(uncert_value_ratio, ttest_p, col = basin)) +
  geom_vline(xintercept = 1) +
  geom_hline(yintercept = 0.05) +
  geom_point() +
  scale_y_log10() +
  scale_color_brewer(palette = "Dark2") +
  facet_grid(parameter ~ estimate)


budget_ensemble_significance %>%
  mutate(across(4:6, round, 2)) %>%
  mutate(across(7, signif, 2)) %>%
  write_csv(here::here("output/publication/Table_budget_signficance_stats.csv"))
...
ggsave(path = here::here("output/publication"),
       filename = "Fig_dcant_profiles_and_budget_depth_layer.png",
       height = 12,
       width = 8)
...
tcant_ts <- tcant_ts %>%
  ...
  mutate(
    ...
    dcant_sd_int = sqrt(dcant_sd ^ 2 + lag(dcant_sd) ^ 2),
    tcant_sd = sqrt(dcant_sd_int ^ 2 + lag(dcant_sd_int) ^ 2)
  ) %>%
  mutate(
    tcant_sd = case_when(
      year == 1800 ~ 0,
      year == 1994 ~ dcant_sd,
      year == 2004 ~ dcant_sd_int,
      TRUE ~ tcant_sd
    )
  ) %>%
  mutate(dcant_sd_int = case_when(
    year == 1800 ~ 0,
    year == 1994 ~ 0,
    year == 2004 ~ dcant_sd,
    TRUE ~ dcant_sd_int
...
tcant_max =
  ...
  tcant_ts_zoom %>% pull(dcant_sd_int) %>% max()
...
dcant_type_box %>%
  ...
  geom_jitter(aes(fill = type),
              ...
              alpha = 0.5) +
  scale_fill_manual(
    values = c("black", "#E69F00", "#56B4E9", "pink"),
    name = "Data source",
    guide = "none"
  ) +
  scale_color_manual(
