...
RECCAP2_fgco2_glob_all_annual_cum_1994 <- RECCAP2_fgco2_glob_all_annual_obs %>%
  ...
  mutate(fgco2_glob = if_else(year == 1994, 0, fgco2_glob),
         fgco2_glob_corr = 1.1*fgco2_glob + 0.62) %>% 
  arrange(year) %>%
  group_by(product) %>%
...
tcant_ts_zoom %>%
  ...
  geom_line(data = RECCAP2_fgco2_glob_all_annual_cum_1994 %>%
              filter(product == "UOEX_Wat20_1985_2019_v20211204"),
            aes(pCO2, fgco2_glob_corr_cum, group = product, col = "Watson x 1.1"),
            linetype = 2) +
  geom_line(data = SeaFlux_fgco2_glob_all_annual_cum_1994,
            aes(pCO2, fgco2_glob_corr_cum, group = product, col = "SeaFlux")) +
  geom_line(aes(col = "eMLR(C*) estimate")) +
  geom_point(aes(col = "eMLR(C*) estimate"),
             shape = 21, fill = "white") +
