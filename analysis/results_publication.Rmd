...
```{r read_zonal_section_files_ensemble}
...
for (i_Version_IDs in Version_IDs_ensemble) {
  ...
  if (exists("dcant_zonal_all_ensemble")) {
    dcant_zonal_all_ensemble <- bind_rows(dcant_zonal_all_ensemble, dcant_zonal)
  }
  
  if (!exists("dcant_zonal_all_ensemble")) {
    dcant_zonal_all_ensemble <- dcant_zonal
  }

  if (exists("dcant_profile_all_ensemble")) {
    dcant_profile_all_ensemble <- bind_rows(dcant_profile_all_ensemble, dcant_profile)
  }
  
  if (!exists("dcant_profile_all_ensemble")) {
    dcant_profile_all_ensemble <- dcant_profile
  }

  if (exists("dcant_profile_basin_MLR_all_ensemble")) {
    dcant_profile_basin_MLR_all_ensemble <- bind_rows(dcant_profile_basin_MLR_all_ensemble, dcant_profile_basin_MLR)
  }
  
  if (!exists("dcant_profile_basin_MLR_all_ensemble")) {
    dcant_profile_basin_MLR_all_ensemble <- dcant_profile_basin_MLR
  }

  if (exists("dcant_budget_basin_AIP_layer_all_ensemble")) {
    dcant_budget_basin_AIP_layer_all_ensemble <-
      bind_rows(dcant_budget_basin_AIP_layer_all_ensemble,
                dcant_budget_basin_AIP_layer)
  }
  
  if (!exists("dcant_budget_basin_AIP_layer_all_ensemble")) {
    dcant_budget_basin_AIP_layer_all_ensemble <- dcant_budget_basin_AIP_layer
  }

  if (exists("dcant_budget_basin_MLR_layer_all_ensemble")) {
    dcant_budget_basin_MLR_layer_all_ensemble <-
      bind_rows(dcant_budget_basin_MLR_layer_all_ensemble,
                dcant_budget_basin_MLR_layer)
  }
  
  if (!exists("dcant_budget_basin_MLR_layer_all_ensemble")) {
    dcant_budget_basin_MLR_layer_all_ensemble <- dcant_budget_basin_MLR_layer
  }

  if (exists("dcant_zonal_bias_all_ensemble")) {
    dcant_zonal_bias_all_ensemble <- bind_rows(dcant_zonal_bias_all_ensemble, dcant_zonal_bias)
  }
  
  if (!exists("dcant_zonal_bias_all_ensemble")) {
    dcant_zonal_bias_all_ensemble <- dcant_zonal_bias
  }
...
dcant_zonal_all_ensemble <- full_join(dcant_zonal_all_ensemble,
                                      params_local_all_ensemble)

dcant_profile_all_ensemble <- full_join(dcant_profile_all_ensemble,
                                        params_local_all_ensemble)
...
dcant_budget_basin_AIP_layer_all_ensemble <-
  full_join(dcant_budget_basin_AIP_layer_all_ensemble,
            params_local_all_ensemble)
...
dcant_budget_basin_MLR_layer_all_ensemble <-
  full_join(dcant_budget_basin_MLR_layer_all_ensemble,
            params_local_all_ensemble)

dcant_zonal_bias_all_ensemble <-
  full_join(dcant_zonal_bias_all_ensemble,
            params_local_all_ensemble)
...
```{r read_RECCAP2_flux_products_models, eval=FALSE}
...
dcant_inv_all_ensemble_mean %>%   p_map_cant_inv(var = "dcant_sd",
...
```{r zonal_mean_storage_Atlantic, fig.asp=1.5}
...
```{r concentration_profiles_MLR_basins_ensemble, fig.asp=0.9}
...
dcant_budget_basin_MLR_all_ensemble %>%
  filter(data_source != "mod_truth",
         # basin == "S_Pacific",
         period != "1994 - 2014") %>%
  ggplot(aes(period, dcant, col=Version_ID_group, group = NA)) +
  scale_fill_brewer(palette = "Set1") +
  scale_color_brewer(palette = "Set1") +
  geom_dots(binwidth = 0.5, side = "both", shape=16) +
...
```{r mean_tcant_over_atm_co2_incl_fluxes, fig.asp=0.6, eval=FALSE}
