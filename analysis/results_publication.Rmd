...
dcant_budget <- expression(atop(Delta * C["ant"],
                                (PgC)))
```
...
```{r cases_absolute_basins_hemisphere_alluvial, fig.asp=0.9}
...
dcant_budget_basin_MLR_all_plot %>%
  ...
  stat_stratum(geom = "text",
               decreasing = FALSE,
               aes(label = paste(100*round(after_stat(prop), 2), "%"))) +
...
dcant_budget_basin_MLR_all_plot %>%
  ...
  geom_text(stat = "stratum", aes(label = paste(100*round(after_stat(prop), 2), "%"))) +
  ggrepel::geom_label_repel(
    data = dcant_budget_basin_MLR_all_plot %>% filter(period == "2004 - 2014"),
    stat = "stratum",
    size = 4,
