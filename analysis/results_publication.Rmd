...
df_regional <-
  ...
  pivot_longer(all_of(MLR_basins_sensitivity),
               names_to = "MLR_basins",
               values_to = "dcant") %>% 
  mutate(delta_dcant = !!sym(MLR_basins_reference) - dcant) %>% 
...
dcant_budget_basin_MLR_all_uncertainty_contributions %>%
  ...
  labs(y = expression(Delta*Cant~inventory~offset~from~standard~case~(Pg~C~dec^{-1}))) +
  theme(
    axis.title.x = element_blank(),
    axis.text.x = element_blank(),
