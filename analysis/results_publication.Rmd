...
beta_inv_label <- expression(beta[area]~inventory~(mol~m^{-2}~ppm^{-1}))
...
beta_CI_label <- expression(atop(beta[area],
                                 (mol ~ m ^ {-2} ~ ppm ^ {-1})))
...
#                                        -1
#                                      }))))
legend_title <- "ΔC<sub>ant</sub> column inventory<br>1σ uncertainty<br>(mol m<sup>-2</sup> dec<sup>-1</sup>)"
...
#                                 })))
legend_title <-
  "ΔΔC<sub>ant</sub> column inventory<br>(mol m<sup>-2</sup> dec<sup>-1</sup>)"
...
dcant_budget_lat_grid_all_ensemble_uncertainty <-
  dcant_budget_lat_grid_all_ensemble %>%
  filter(period %in% two_decades,
         data_source != "mod_truth") %>%
  select(lat_grid,
         period,
         ...
         Version_ID_group,
         dcant) %>%
  mutate(dcant = dcant / 5)

# dcant_budget_lat_grid_all_ensemble_uncertainty <-
#   full_join(dcant_inv_all_ensemble,
#             basinmask %>% mutate(area = earth_surf(lat, lon)) %>% select(-basin))
#   
# dcant_budget_lat_grid_all_ensemble_uncertainty <-
#   dcant_budget_lat_grid_all_ensemble_uncertainty %>%
#   filter(period %in% two_decades,
#          data_source != "mod_truth") %>%
#   mutate(dcant = dcant * area * 12 * 1e-15) %>%
...
dcant_budget_lat_grid_all_ensemble_uncertainty_global <-
  dcant_budget_lat_grid_all_ensemble_uncertainty %>%
  group_by(lat_grid, period, data_source, MLR_basins, Version_ID_group) %>%
  summarise(dcant = sum(dcant)) %>%
  ungroup() %>%
  mutate(basin_AIP = "Global")
...
rm(dcant_budget_lat_grid_all_ensemble_uncertainty_global)

  
dcant_budget_lat_grid_uncertainty <- additive_uncertainty(dcant_budget_lat_grid_all_ensemble_uncertainty)

delta_dcant_budget_lat_grid_uncertainty <- dcant_budget_lat_grid_uncertainty[[3]]
dcant_budget_lat_grid_uncertainty_contributions <- dcant_budget_lat_grid_uncertainty[[2]]
dcant_budget_lat_grid_uncertainty <- dcant_budget_lat_grid_uncertainty[[1]]
...
p_lat_int_basin <-
dcant_budget_lat_grid_uncertainty %>%
  filter(data_source == "obs") %>%
  arrange(lat_grid) %>%
  ggplot(aes(std_case, lat_grid,
             xmin = std_case - sd,
             xmax = std_case + sd)) +
  geom_path(aes(col = period)) +
  geom_ribbon(aes(fill = period), alpha = 0.3) +
  scale_y_continuous(limits = c(-78,65), breaks = seq(-80, 80, 20),
                     expand = c(0,0)) +
  scale_fill_manual(values = fill_scale) +
  scale_color_manual(values = fill_scale) +
  labs(y = "Latitude (°N)",
       x = expression(Delta*C[ant]~inventory~per~"1°"~latitude~bin~(Pg~C~dec^{-1}))) +
  facet_wrap( ~ basin_AIP, scales = "free_x") +
  theme(legend.title = element_blank(),
        legend.position = "bottom")


p_lat_int_basin_delta <-
  delta_dcant_budget_lat_grid_uncertainty %>%
  filter(data_source == "obs") %>%
  arrange(lat_grid) %>%
  ggplot(aes(delta_dcant, lat_grid,
  ...
  scale_y_continuous(limits = c(-78,65), breaks = seq(-80, 80, 20),
                     expand = c(0,0)) +
  labs(y = "Latitude (°N)",
       x = expression(Delta*Delta*C[ant]~inventory~per~"1°"~latitude~bin~(Pg~C~dec^{-1}))) +
  facet_wrap( ~ basin_AIP, scales = "free_x") +
  theme(legend.title = element_blank())

p_lat_int_basin + p_lat_int_basin_delta


p_lat_int_basin <-
dcant_budget_lat_grid_uncertainty %>%
  filter(data_source == "obs") %>%
  complete(lat_grid, period, basin_AIP, fill = list(std_case = 0)) %>%
  mutate(std_case = if_else(std_case < 0, 0, std_case)) %>% 
  ggplot(aes(std_case, lat_grid,
             xmin = std_case - sd,
             xmax = std_case + sd)) +
  geom_area(data = . %>% filter(basin_AIP != "Global"),
            aes(fill = basin_AIP), orientation = "y") +
  geom_path(data = . %>% filter(basin_AIP == "Global"),
            aes(col = basin_AIP)) +
  # geom_path(data = . %>% filter(basin_AIP == "Global"),
  #           aes(x = std_case + sd), linetype = 3) +
  # geom_path(data = . %>% filter(basin_AIP == "Global"),
  #           aes(x = std_case - sd), linetype = 3) +
  # geom_ribbon(data = . %>% filter(basin_AIP == "Global"),
  #             alpha = 0.3) +
  scale_y_continuous(limits = c(-78,65), breaks = seq(-80, 80, 20),
                     expand = c(0,0)) +
  ...
  labs(y = "Latitude (°N)",
       x = expression(Delta*C[ant]~inventory~per~"5°"~latitude~bin~(Pg~C~dec^{-1}))) +
  facet_wrap(~ period, ncol = 1) +
  theme(legend.title = element_blank())
...
delta_dcant_budget_lat_grid_uncertainty %>%
  ...
  scale_color_manual(values = "grey20") +
  labs(y = "Latitude (°N)",
       x = expression(Delta*Delta*C[ant]~inventory~per~"5°"~latitude~bin~(Pg~C~dec^{-1}))) +
  theme(legend.title = element_blank())
...
p_lat_int_basin + guide_area() + p_lat_int_basin_delta +
...
dcant_budget_lat_grid_uncertainty %>%
  ...
  arrange(lat_grid) %>%
  ggplot(aes(std_case, lat_grid)) +
  geom_ribbon(aes(xmin = std_case - 2*sd,
                  xmax = std_case + 2*sd),
              alpha = 0.2) +
  geom_ribbon(aes(xmin = std_case - sd,
                  xmax = std_case + sd),
              alpha = 0.4) +
  geom_path()+
  scale_y_continuous(limits = c(-78,65),
                     breaks = seq(-60, 60, 30),
                     labels = c("60°S","30°S","Eq.","30°N","60°N"),
                     expand = c(0,0)) +
  scale_x_continuous(limits = c(-0.05,0.55), breaks = seq(0, 3, 0.2),
                     expand = c(0,0),
                     position = "top") +
  scale_fill_manual(values = fill_scale) +
  scale_color_manual(values = fill_scale) +
  labs(x = expression(atop(Zonal~integrals,
                           (Pg~C~"°lat"^{-1}~dec^{-1})))) +
  theme_classic() +
...
dcant_budget_lat_grid_uncertainty %>%
  ...
  scale_fill_manual(values = fill_scale) +
  scale_color_manual(values = fill_scale) +
  labs(x = expression(atop(Zonal~integral,
                           (Pg~C~dec^{-1}~"°lat"^{-1})))) +
  theme_classic() +
...
p_zonal_int_delta <-
  ...
  geom_vline(xintercept = 0,
             linewidth = 0.5,
             col = "grey50") +
  geom_ribbon(aes(xmin = delta_dcant - 2 * RSS,
                  xmax = delta_dcant + 2 * RSS),
              alpha = 0.2) +
  geom_ribbon(aes(xmin = delta_dcant - RSS,
                  xmax = delta_dcant + RSS),
              alpha = 0.4) +
  geom_path() +
  scale_y_continuous(
...
ggsave(path = here::here("output/publication"),
       filename = "Fig_dcant_column_inventory_zonal_integral.png",
       height = 200,
       width = 42,
       units = "mm",
       dpi = 600,
...
dcant_inv_all %>%
  filter(data_source == "obs",
         period == "1994 - 2004") %>%
  select(lon, lat, dcant) %>% 
  ggplot(aes(lon, lat, fill = cut(dcant, breaks))) +
  geom_tile() +
...
```{r dcant_column_inventory_maps_robinson}
...
# legend_title <- expression(atop(Delta * C["ant"]~column~inventory,
#                                 (mol ~ m ^ {-2} ~ dec ^ {-1})))
legend_title <- "ΔC<sub>ant</sub> column inventory<br>(mol m<sup>-2</sup> dec<sup>-1</sup>)"
...
# legend_title <- expression(atop(Delta * Delta * C["ant"]~column~inventory,
#                                 (mol ~ m ^ {
#                                   -2
#                                 } ~ dec ^ {
#                                   -1
#                                 })))
legend_title <- "ΔΔC<sub>ant</sub> column inventory<br>(mol m<sup>-2</sup> dec<sup>-1</sup>)"
...
# wrap_plots(
#   wrap_plots(
#     (p_zonal_int_dec1 | p_dcant_inv_dec1) + plot_layout(widths = c(1, 8)),
#     (p_zonal_int_dec1 | p_dcant_inv_dec2) + plot_layout(widths = c(1, 8))) + 
#     plot_layout(ncol = 1,
#                 guides = "collect"),
#   (p_zonal_int_dec1 | p_dcant_inv_delta) + plot_layout(widths = c(1, 8))) +
#   plot_layout(ncol = 1,
#               heights = c(2,1)) +
#   plot_annotation(tag_levels = 'a')
# 
...
```{r dcant_column_inventory_maps_robinson_presentation}
...
#                                   -1
#                                 })))
legend_title <- "ΔΔC<sub>ant</sub> column inventory bias<br>(mol m<sup>-2</sup> dec<sup>-1</sup>)"
...
dcant_inv_all %>%
  ...
  arrange(period) %>% 
  group_by(lon, lat) %>% 
  reframe(delta_dcant_bias = dcant_bias - lag(dcant_bias)) %>% 
  drop_na() %>% 
  p_map_cant_inv(
...
legend_title <- "β<sub>area</sub><br>(mol m<sup>-2</sup> ppm<sup>-1</sup>)"
...
```{r write_dcant_column_inventory_files_for_submission}

library(units)
install_unit(symbol = "decade", def = "10 years")
# remove_unit(symbol = "decade")
...
dcant_inv_out <- dcant_inv_out %>%
  select(lon,
         ...
         Version_ID_group,
         dcant) %>%
  rename(MLR_basin_mask = MLR_basins,
         MLR_configuration = Version_ID_group,
         dcant_inv = dcant)
...
dcant_inv_out_stars <- st_as_stars(dcant_inv_out,
                                   dims = c("lon", "lat", "period", "MLR_basin_mask", "MLR_configuration"))
...
dcant_inv_out_stars <-
  dcant_inv_out_stars %>%
  mutate(dcant_inv = set_units(dcant_inv, mol / m ^ 2 / decade))
...
ggplot() +
  ...
  facet_grid(period ~ MLR_configuration) +
  scale_fill_viridis_b(breaks = c(-Inf, seq(0,16,2), Inf),
                       na.value = "transparent") +
  coord_quickmap()
...
```{r write_dcant_column_inventory_uncertainty_files_for_submission}
...
dcant_inv_uncertainty_out <- dcant_inv_uncertainty_out %>% 
  mutate(lon = if_else(lon > 180,
                       lon - 360,
                       lon))

dcant_inv_uncertainty_out <- dcant_inv_uncertainty_out %>% 
  rename(dcant_inv = dcant,
         dcant_inv_uncertainty = uncertainty)
...
dcant_inv_uncertainty_out_stars <-
  dcant_inv_uncertainty_out_stars %>%
  mutate(
    dcant_inv = set_units(dcant_inv, mol / m ^ 2 / decade),
    dcant_inv_uncertainty = set_units(dcant_inv_uncertainty, mol / m ^ 2 / decade)
  )
...
ggplot() +
  geom_stars(data = dcant_inv_uncertainty_out_stars,
             aes(fill = dcant_inv_uncertainty)) +
  facet_grid(period ~ .) +
  scale_fill_viridis_b(
    breaks = c(seq(0, 8, 2), Inf),
    na.value = "transparent"
  )

ggplot() +
  geom_stars(data = dcant_inv_uncertainty_out_stars,
             aes(fill = abs(dcant_inv) - dcant_inv_uncertainty)) +
  facet_grid(period ~ .) +
  scale_fill_binned_divergingx(
    breaks = c(-Inf, 0 , Inf),
    palette = "RdBu",
...
st_get_dimension_values(dcant_inv_uncertainty_read_mdim,
                        "lat")
...
dcant_inv_uncertainty_read_tidync %>%
  tidync::hyper_tibble() %>%
  ggplot(aes(lon, lat)) +
  geom_raster(aes(fill = abs(dcant_inv) - dcant_inv_uncertainty)) +
  facet_grid(period ~ .) +
  scale_fill_binned_divergingx(
    breaks = c(-Inf, 0 , Inf),
    palette = "RdBu",
...
dist_SO_Atl_boundary <- section_global_coordinates %>%
  filter(lat == min(lat)) %>%
  pull(dist) %>%
  min()


dist_SO_Pac_boundary <- section_global_coordinates %>%
  filter(lat == min(lat)) %>%
  pull(dist) %>%
  max()

length_SO <- dist_SO_Pac_boundary - dist_SO_Atl_boundary
...
dcant_global_section_all_uncertainty <-
  ...
  mutate(
    dist = case_when(
      dist > dist_SO_Atl_boundary &
        dist < dist_SO_Pac_boundary ~ dist_SO_Atl_boundary + (dist - dist_SO_Atl_boundary) * SO_compression,
      dist >= dist_SO_Pac_boundary ~ dist - length_SO * (1 - SO_compression),
      TRUE ~ dist
    )
  ) 
...
dcant_global_section_all_uncertainty <-
  ...
  select(dist,
         ...
         Version_ID_group,
         dcant) %>%
  mutate(dist_grid = as.numeric(as.character(cut(dist, seq(0,50,1), seq(0.5,50,1)))),
  # depth_grid = depth
...
legend_title <- "ΔC<sub>ant</sub><br>(µmol kg<sup>-1</sup> dec<sup>-1</sup>)"
...
legend_title <- "ΔΔC<sub>ant</sub><br>(µmol kg<sup>-1</sup> dec<sup>-1</sup>)"
...
dcant_budget_basin_MLR_all_uncertainty_contributions %>%
  ...
  labs(y = expression(Absolute~Delta*C[ant]~inventory~offset~(Pg~C~dec^{-1}))) +
  theme(
    axis.title.x = element_blank(),
    axis.text.x = element_blank(),
...
ggsave(
  path = here::here("output/publication"),
  filename = "Fig_dcant_inventory_uncertainty_contributions.png",
  height = 130,
  width = 190,
  units = "mm",
  dpi = 600,
...
```{r dcant_budget_basin_hemisphere_additive_uncertainty_waterfall, fig.asp=1}
...
dcant_waterfall <-
  dcant_waterfall %>%
  mutate(
    fill = if_else(period != "Dec. diff.", period, "Dec. difference"),
...
geom_errorbar(
  ...
  scale_fill_manual(
    ...
    label = c("Dec. difference" = expression(Delta*Delta*C[ant]))
  ) +
...
beta_scaling <- beta_scaling %>% 
  ...
  # mutate(scaling_factor = 1 / delta_pCO2) %>% 
...
p_tcant_year <- tcant_ts %>%
  ...
  scale_y_continuous(name = expression(Total ~ oceanic ~ C[ant] ~ (Pg~C)),
                     sec.axis = sec_axis( ~ (. / beta + co2_atm_pi_value),
                                          name = expression(Atmospheric ~ CO[2] ~ (ppm)))) +
  scale_x_continuous(name = "Year", breaks = seq(1800, 2000, 50)) +
  scale_linewidth(
    range = c(0.5, 1),
    breaks = c(0.5, 1),
    labels = c("\u00B1 2σ", "\u00B1 1σ"),
    name = "Interval uncertainty"
  )+
...
```{r seperate_Watson_from_GCB, eval=FALSE}
...
## Ocean interior dcant

```{r write_ocean_interior_dcant}
...
dcant_3d_out_stars <-
  dcant_3d_out_stars %>%
  mutate(
    dcant = set_units(dcant, µmol / kg / decade)
  )
...
volume_mask_out_stars <- volume_mask_out_stars %>%
  mutate(volume = set_units(volume, m ^ 3))
...
basinmask_all <- basinmask_all %>% 
  select(lon, lat, MLR_basin_mask = MLR_basins, sub_basin_name = basin)

basinmask_all <- basinmask_all %>% 
  group_by(MLR_basin_mask) %>% 
  mutate(sub_basin_name = as.numeric(as.factor(sub_basin_name))) %>% 
  ungroup()
...
ggplot() +
  geom_stars(data = basinmask_all_read_mdim,
             aes(fill = as.factor(sub_basin_name))) +
  facet_wrap(~ MLR_basin_mask) +
  scale_fill_muted(na.value = "transparent")

surface_area_mask <- basinmask_all %>% 
  filter(MLR_basin_mask == "1") %>% 
  select(-c(sub_basin_name,MLR_basin_mask))

surface_area_mask <- surface_area_mask %>%
  mutate(lon = if_else(lon > 180,
                       lon - 360,
                       lon))
...
surface_area_mask_out_stars <- surface_area_mask_out_stars %>% 
  mutate(surface_area = set_units(surface_area, m ^ 2))
