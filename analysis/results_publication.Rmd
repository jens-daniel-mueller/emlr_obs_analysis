...
params_local_all_ensemble <-
  ...
  mutate(
    ...
    Version_ID_group = case_when(
      ...
      Version_ID_group == "h" ~ "Surface eMLR(C*)",
...
df_factorial_stat <-
  ...
  select(-Version_ID_group) %>% 
  group_by(across(-sd)) %>% 
  summarise(sd = sqrt(sum(sd^2)) * sd_factor) %>% 
  ungroup()
...
df_stat <- df_stat %>%
  mutate(signif = if_else(abs(std_case) >= sd,
                          1,
                          0))
...
```{r inventory_maps_absolute_obs_10000, fig.asp=1.2}
...
scaling_uncertainty <-
  ...
  filter(basin == "Global") %>%
  mutate(
    sd = std_case * (dcant_scaling - 1) * dcant_scaling_uncertainty / sd_factor,
...
dcant_beta_basin_MLR_all_uncertainty_significance %>%
  ...
  mutate(
    ...
    `Decadal change` = paste0(delta, " ± ", RSS, 
                              if_else(abs(delta) > RSS/sd_factor, " *",""), 
...
p_tcant_ts_zoom <-
  ...
  geom_ribbon(
    aes(
      ymin = tcant - dcant_sd / sd_factor,
      ymax = tcant + dcant_sd / sd_factor
    ),
    alpha = 0.3,
    fill = tcant_col
  ) +
...
dcant_type_box %>%
 ...
 geom_crossbar(
    ...
    aes(
      x = period,
      y = dcant_mean,
      ymin = dcant_mean - dcant_sd/sd_factor,
      ymax = dcant_mean + dcant_sd/sd_factor,
      col = type,
      fill = type
    ), width = 0.7, alpha = 0.4
  ) +
...
Ocean_Sink_cum_1994_ensemble_mean <- Ocean_Sink_cum_1994 %>% 
  ...
  summarise(fgco2_glob_cum_total_mean = mean(fgco2_glob_cum_total),
            fgco2_glob_cum_total_sd = sd(fgco2_glob_cum_total)*sd_factor) %>% 
  ungroup()
...
dcant_per_emissions <- left_join(GCB_Cant_emissions,
          tcant_ts_zoom) %>%
  mutate(
    cant_emissions_sd_rel = 0.09,
    dcant_sd_rel = dcant_sd_int / dcant_mean,
    dcant_per_emissions_sd_rel = sqrt(dcant_sd_rel^2 + cant_emissions_sd_rel^2),
    dcant_per_emissions = 100 * dcant_mean / cant_emissions,
    dcant_per_emissions_sd = dcant_per_emissions_sd_rel * dcant_per_emissions
  ) %>%
  select(year, cant_emissions, dcant_mean, dcant_sd, dcant_per_emissions, dcant_per_emissions_sd)
...
dcant_per_emissions %>% 
  mutate(delta_dcant_per_emissions = dcant_per_emissions - lag(dcant_per_emissions),
         delta_dcant_per_emissions_sd = sqrt(dcant_per_emissions_sd^2 + lag(dcant_per_emissions_sd)^2))
