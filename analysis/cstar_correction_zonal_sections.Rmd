...
dcant_zonal_all %>%
  ...
  map(
    ~ p_section_zonal_continous_depth(
      ...
      facet_grid(cstar_corr_316N199 ~ data_source)
  )
...
dcant_zonal_bias_all %>%
  ...
  map(
    ~ p_section_zonal_continous_depth(
      ...
      facet_grid(cstar_corr_316N199 ~ period)
  )
...
dcant_zonal_bias_all %>%
  ggplot(aes(dcant_bias, col = cstar_corr_316N199)) +
...
dcant_profile_all %>%
  ...
  map(
    ~ ggplot(data = .x,
      ...
      facet_grid(cstar_corr_316N199 ~ basin_AIP)
  )
...
dcant_profile_all %>%
  ...
  map(
    ~ ggplot(data = .x,
             aes(
               dcant, depth,
               col = cstar_corr_316N199, fill = cstar_corr_316N199
             )) +
      geom_hline(yintercept = params_global$inventory_depth_standard) +
      geom_vline(xintercept = 0) +
      geom_path() +
  ...
  map(
    ~ ggplot(
      ...
      aes(
        ...
        depth,
        col = period,
        group = interaction(cstar_corr_316N199, period)
      )
    ) +
      geom_hline(yintercept = params_global$inventory_depth_standard) +
      geom_vline(xintercept = 0) +
