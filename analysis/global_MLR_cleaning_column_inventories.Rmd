  ...
  map(
    ~ p_map_cant_inv(df = .x,
                     ...
                     subtitle_text = paste("period:",
                                           unique(.x$period))) +
      facet_grid(target_global_rmse_max ~ data_source)
  )
...
dcant_inv_bias_all %>%
  ...
  facet_grid(target_global_rmse_max ~ period)
...
dcant_inv_ensemble_member <- dcant_inv_bias_all %>% 
  group_by(period, target_global_rmse_max) %>% 
  summarise(dcant_sd = sd(dcant_bias)) %>% 
  ungroup()

dcant_inv_ensemble_member %>% 
  ggplot(aes(target_global_rmse_max, dcant_sd, col = period)) +
  geom_point()
